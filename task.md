...
- Verified: `tsc`, lint and `npm run build` clean. Not exercised in a browser (needs sign-in). Manual pass, in order: (1) online: New job → Complete → Jobs shows Syncing briefly, then Completed; (2) DevTools → Network → Offline: New job → Complete → "Job saved" → list shows the job as Pending sync at the top; back online → Syncing → Completed, PDF-less Job Detail shows its data; (3) reload mid-Step 2 → the draft is back; (4) two devices/browsers signed in: edit WO-10029 on both, sync the second → 02g appears on the second, try both buttons; (5) airplane mode while a photo uploads → Complete → back online → photo appears in Job Detail.
