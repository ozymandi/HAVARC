...
**2026-09-13, client feedback round 1 (PDF contrast).** The client marked light captions in two Service Report PDFs. Pushed in `cec6827`: equipment unit labels and CUSTOMER / TECHNICIAN SIGNATURE captions → `text-icon` (near-dark); hero info card (CUSTOMER, ADDRESS, DATE, PHONE, WORK ORDER) labels → white, its translucent panel → solid `bg-brand-strong` navy. Not changed, Yaroslav's call pending: header contact lines (phone/email/address on the gradient, client's mark "1"), the running-header pill on pages 2+, the invoice PDF's signature-field labels (same `ink-faint`). Found in the client's second PDF: work order `WO-19fd1080` — the fallback in `src/data/jobDraft.ts` (`WO-` + 8 chars of the job UUID) leaked into a completed job, most likely a job started offline before the next number was cached; needs a fix (assign the real number on sync, like invoice numbers).
