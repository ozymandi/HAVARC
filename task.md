...
**2026-09-13, client feedback round 1 (PDF contrast).** The client marked light captions in two Service Report PDFs. Pushed in `cec6827`: equipment unit labels and CUSTOMER / TECHNICIAN SIGNATURE captions → `text-icon` (near-dark); hero info card (CUSTOMER, ADDRESS, DATE, PHONE, WORK ORDER) labels → white, its translucent panel → solid `bg-brand-strong` navy. Round 2 (same day): header contact lines weight 500 + faint text shadow; running-header pill on pages 2+ → solid `bg-brand-strong`; every remaining `ink-faint` caption in both PDFs (invoice signature fields, Parts line, photo captions) → `text-icon`. No faint text is left in `src/pdf`. Colours confirmed by Yaroslav 2026-09-14. Stored PDFs keep the old look (files in Storage); the client opened an old one — Yaroslav created a fresh test job for him the same day. To refresh an old job's PDFs: Edit job → Complete, or Save invoice (regenerates both and re-sends the email). Found in the client's second PDF: work order `WO-19fd1080` — the fallback in `src/data/jobDraft.ts` (`WO-` + 8 chars of the job UUID) leaked into a completed job, most likely a job started offline before the next number was cached; needs a fix (assign the real number on sync, like invoice numbers).
...
- Handover checklist (in addition to the Handover section): ~~delete test jobs~~ done 2026-09-12 (test jobs, test customers and their Storage files removed; WO-10029 restored to its seed content; invoice counter back at 649 — the DB is the seed again: 5 jobs, 5 customers, Storage empty); set `settings.next_invoice_number` to the client's real next number and the invoice prefix; ~~notify_email~~ set to `info@havarcservices.com` (from the client's website footer) on 2026-09-12 for the client's test round — confirm with the client; the company contact email printed on PDFs is still `hav.arcservices@gmail.com` from the brief (Settings → Email if the client wants the domain address instead); switch Brevo to the client's domain/sender (their domain is `havarcservices.com`, site by MediaLinkers — DNS access needed for the two Brevo records); move Supabase, GitHub, Vercel to the client's accounts; then re-run (2) once on the client's phone.
