...
**2026-09-13, client feedback round 1 (PDF contrast).** The client marked light captions in two Service Report PDFs. Pushed in `cec6827`: equipment unit labels and CUSTOMER / TECHNICIAN SIGNATURE captions → `text-icon` (near-dark); hero info card (CUSTOMER, ADDRESS, DATE, PHONE, WORK ORDER) labels → white, its translucent panel → solid `bg-brand-strong` navy. Round 2 (same day): header contact lines weight 500 + faint text shadow; running-header pill on pages 2+ → solid `bg-brand-strong`; every remaining `ink-faint` caption in both PDFs (invoice signature fields, Parts line, photo captions) → `text-icon`. No faint text is left in `src/pdf`. Found in the client's second PDF: work order `WO-19fd1080` — the fallback in `src/data/jobDraft.ts` (`WO-` + 8 chars of the job UUID) leaked into a completed job, most likely a job started offline before the next number was cached; needs a fix (assign the real number on sync, like invoice numbers).
