...
- Verified locally: WO-10029 report (3 pages) and invoice (2 pages) rendered as exact 612×792 pt Letter pages from real data; Job Detail Documents rows and the generating state exercised in the browser. Not yet verified on Vercel — needs `SUPABASE_SECRET_KEY` in the project env (Secret, Production + Preview), then a Complete on prod.

Next: step 7 — Share sheet with the real files (Web Share API on phones, downloads on desktop). Then step 8 part 2 (email the PDFs after generation), step 9 (autoreload on new version, Work performed checklist from step data), step 10 (tests, handover).
