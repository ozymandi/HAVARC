...
3. Own sender (plan step 8, pulled forward 2026-09-12): Brevo SMTP relay declared in `supabase/config.toml` (`[auth.email.smtp]`, credentials via `env(SMTP_USER|SMTP_PASS|SMTP_SENDER)` from `.env.local`, loaded by `dotenv-cli` in the config scripts) and a branded recovery email (`supabase/templates/recovery.html`, subject "HAV'ARC — Reset your password"). Waiting on: Yaroslav creates the Brevo account, verifies a sender address, generates an SMTP key, fills `.env.local`, runs `npm run config:push`. Later swap to the client's own domain/sender = change `SMTP_SENDER` (and Brevo domain auth) + push.
